...
- Supported aliases include `PXA`, `Plan Examination by Area`, and `Plan Examination v2`.
...
- `Plan Examination by Area` and `Plan Examination v2` resolve through the same PXA transaction profile and document profile.
...
- `2-18A`: documented PE/PXA parity and `Plan Examination by Area` / `Plan Examination v2` alias behavior.
...
- Added workflow-rule regression coverage for `Plan Examination by Area` and `Plan Examination v2` resolving to PXA.
