...
This story adds a transaction-scoped document viewer dockpane. It should be registered as a first-class ArcGIS Pro pane tab, use the whole dockpane content area, list only readable supporting files, and clear or hide itself when the transaction is not loaded or no longer active.
...
2. Given a transaction is loaded, then ArcGIS Pro exposes a dockpane tab titled `Supporting Documents [TR-<transaction-number>]` at the same dockpane level as Contents, Catalog, and Parcel Workflow.
...
6. Given the attachment list contains `.pdf`, then the selected PDF opens in the embedded PDF viewer when available and falls back to an external/open-source state when embedded viewing is unavailable.
7. Given the attachment list contains `.txt`, then the selected text file is rendered read-only in the panel with scrolling.
8. Given the attachment list contains `.doc`, `.docx`, or `.dwg`, then the file can appear in the combo box only if it was copied into the case folder, but the panel may show a preview-unavailable state with Open source and Open in folder actions unless a supported embedded viewer exists.
...
Sally recommendation: make this a document workspace, not another nested card or tab inside the workflow stages. The panel should be a separate ArcGIS Pro dockpane tab that is quiet, direct, and built for inspection: selector at the top, document taking the rest of the space.

```text
ArcGIS Pro dockpane tabs
┌──────────────────────────────────────────────────────────────────────────────┐
│ Contents | Catalog | Parcel Workflow [Compute] | Supporting Documents [TR-1000099999] │
├──────────────────────────────────────────────────────────────────────────────┤
│ Document  [ DOC_PLAN_492949_A.pdf                              v ] [Open] [ ]│
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│                              PDF / TXT VIEWER                                │
│                                                                              │
...
- Do not promise embedded DWG viewing in this story. The safe first behavior is to list copied DWG files and provide clear external-open actions.
...
- [x] Add a transaction-scoped Supporting Documents dockpane tab. (AC: 1, 2, 9, 10)
...
- [x] Implement viewer modes. (AC: 6, 7, 8, 11)
  ...
  - [x] Keep Open source and Open in folder actions available for copied files.
...
- [x] Add full-panel layout and bindings. (AC: 2, 3, 6, 7, 8, 9)
  ...
  - [x] Add file/open controls next to the combo box.
...
- [x] Add automated tests. (AC: 1-12)
  ...
  - [x] XAML/binding smoke test: selector, open actions, empty state, and viewer host are present.
...
- Moved Supporting Documents out of the Parcel Workflow pane and registered it as its own ArcGIS Pro dockpane tab.
