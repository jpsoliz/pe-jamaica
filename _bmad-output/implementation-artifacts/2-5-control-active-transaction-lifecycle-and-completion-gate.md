...
8. Given completion readiness criteria are met and the logged-in user is the user who started/claimed the task, when the user clicks Complete/Finalize, then the add-in calls the Innola completion service operation, records completion success/failure in the Case Folder audit trail and manifest, clears the active transaction gates on success, disables Cancel/Suspend/Finalize for the completed task, shows a completion confirmation popup, and requires a transaction refresh before another task can be completed.
...
- [x] Add lifecycle controls and status to the Transaction Panel and Parcel Workflow pane. (AC: 1, 2, 4, 5, 7, 8, 9)
  ...
  - [x] Successful Finalize must not leave a retry/failure banner when the Case Folder manifest/audit already shows `innola_lifecycle.status = completed`; the UI must show the completed status and disable Cancel/Suspend for that closed task.
